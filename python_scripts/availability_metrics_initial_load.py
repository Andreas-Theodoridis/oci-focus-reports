...
def upload_csv_to_oracle(csv_file, db_user, db_pass, db_dsn):
        ...
        for row in reader:
            try:
                ...
                rows.append((
                    ...
                    row["timestamp"],  # string format, will be converted in SQL
                    row["namespace"],
                    row["compartment_id"],
    ...
    if not rows:
        logging.info("⚠️ No valid rows to upload.")
        return
    ...
    cursor = conn.cursor()
    cursor.arraysize = 1000  # tune as needed for batching
    ...
    insert_sql = f"""
        INSERT INTO {table_name} (
            ...
            :1,
            TO_TIMESTAMP_TZ(:2, 'YYYY-MM-DD"T"HH24:MI:SS.FF3TZH:TZM'),
            :3, :4, :5, :6
        )
    """

    logging.info(f"⬆️ Inserting {len(rows)} rows into {table_name}...")
