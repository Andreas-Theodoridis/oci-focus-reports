...
log_file_pattern= config["idika_compartments_file_name_pattern"]
...
log_filename = os.path.join(log_dir, f"idika_compartments_{timestamp}.log")
latest_log_symlink = os.path.join(log_dir, "latest_idika_compartments.log")
...
os.symlink(f"idika_compartments_{timestamp}.log", latest_log_symlink)
...
def convert_json_to_csv(data):
    ...
    fieldnames = [
        ...
        "Parent ID", "Parent", "Path", "Tenancy_ID"
    ]
...
def upload_csv_to_oracle(csv_data, db_user, db_pass, db_dsn):
    ...
    USING (
        SELECT :1 AS "Compartment ID", :2 AS "Name", :3 AS "Description",
               ...
               :7 AS "Parent", :8 AS "Path", :9 AS "Tenancy_ID"
        FROM dual
    ) src
    ...
    WHEN NOT MATCHED THEN INSERT (
        ...
        "Parent_ID", PARENT, "Path", "Tenancy_ID"
    )
    VALUES (
        src."Compartment ID", src."Name", src."Description", src."Lifecycle State",
        src."Time Created", src."Parent ID", src."Parent", src."Path", src."Tenancy_ID"
    )
    """
...
def main():
    ...
    try:
        ...
        output_path = os.path.join(output_dir, "compartments_idika_data.csv")
