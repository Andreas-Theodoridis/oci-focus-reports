...
if [[ "$USER_RESPONSE" =~ ^[Yy]$ ]]; then
    SCALE_UP_CRON_ENTRY="0 8 * * 1-5 $SCRIPT_DIR/scale_up.sh >> /home/opc/oci-focus-reports/logs/scale_up.log 2>&1"
    grep -F -- "$SCALE_UP_CRON_ENTRY" "$TEMP_CRON" >/dev/null || echo "$SCALE_UP_CRON_ENTRY" >> "$TEMP_CRON"
    echo "✅ Added scale_up.sh to crontab at 08:00 every weekday."
fi
...
read -r USER_RESPONSE
if [[ "$USER_RESPONSE" =~ ^[Yy]$ ]]; then
    SCALE_DOWN_CRON_ENTRY="0 20 * * * $SCRIPT_DIR/scale_down.sh >> /home/opc/oci-focus-reports/logs/scale_down.log 2>&1"
