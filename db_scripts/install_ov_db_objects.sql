...
--------------------------------------------------------
  ...
  CREATE TABLE "OCI_FOCUS_REPORTS"."OCI_AVAILABILITY_METRICS_PY" 
   (	"RESOURCEDISPLAYNAME" VARCHAR2(1000) COLLATE "USING_NLS_COMP", 
...
--------------------------------------------------------
--  DDL for Table OCI_MY_CUSTOM_REPORTS
--------------------------------------------------------

  CREATE TABLE "OCI_FOCUS_REPORTS"."OCI_MY_CUSTOM_REPORTS"
    ("ID" NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER  NOCYCLE  NOKEEP  NOSCALE  NOT NULL ENABLE,
    "REPORT_NAME" VARCHAR2(100) COLLATE "USING_NLS_COMP",
    "WORKLOAD_NAME" VARCHAR2(100) COLLATE "USING_NLS_COMP",
    "COMPARTMENT_ID" CLOB COLLATE "USING_NLS_COMP",
    "OWNER" VARCHAR2(100) COLLATE "USING_NLS_COMP");
...
BEGIN
  ...
  INTO COST_USAGE_TIMESERIES_DAILY (
    ...
    COST, USAGE, CREATED_BY, LAST_REFRESH
  )
  SELECT
  ...
  FROM OCI_FOCUS_REPORTS.FOCUS_REPORTS_PY fr
...
INSERT INTO AI_PROMPT_EXAMPLES (example_order, user_question, oracle_sql)
VALUES (9, 'List the top 10 usernames by cost for the last three months', 
'SELECT NVL(c.CREATED_BY, ''N/A'') AS USERNAME, SUM(c.COST) AS TOTAL_COST FROM COST_USAGE_TIMESERIES_DAILY c WHERE c.DATE_BUCKET >= TRUNC(ADD_MONTHS(SYSDATE, -3), ''MM'') GROUP BY c.CREATED_BY ORDER BY TOTAL_COST DESC FETCH FIRST 15 ROWS ONLY;');

INSERT INTO AI_PROMPT_EXAMPLES (example_order, user_question, oracle_sql)
VALUES (10, 'List the top 10 workloads by cost for the last three months', 
'SELECT w.WORKLOAD_NAME, SUM(c.COST) AS TOTAL_COST FROM COST_USAGE_TIMESERIES_DAILY c JOIN OCI_WORKLOADS w ON c.OCI_COMPARTMENTID = TO_CHAR(w.VALUE) WHERE c.DATE_BUCKET >= TRUNC(ADD_MONTHS(SYSDATE, -3), ''MM'') GROUP BY w.WORKLOAD_NAME ORDER BY TOTAL_COST DESC FETCH FIRST 15 ROWS ONLY;');
...
BEGIN
    ...
    FOR date_rec IN cur_dates LOOP
        v_html := v_html || '<tr><td>' || date_rec.day_label || '</td>';
        FOR i IN 1 .. v_idx LOOP
...
create or replace FUNCTION OCI_FOCUS_REPORTS.cost_pivot_dynamic_weekly_html (
    ...
    CURSOR cur_dates IS
        SELECT DISTINCT TO_CHAR(DATE_BUCKET, 'DD-Mon') || ' (W' || TO_CHAR(DATE_BUCKET, 'IW') || ')' AS week_label, DATE_BUCKET
...
BEGIN
    ...
    v_html := '<div class="pivot-scroll"><table id="pivot-table" class="pivot-table" border="1"><thead><tr><th rowspan="3">Week</th>';
    ...
    v_sql := '
        ...
        || add_filter('RESOURCETYPE', p_resource_type)
        || add_filter('RESOURCEID', p_resource_name)
        || ' GROUP BY TO_CHAR(DATE_BUCKET, ''DD-Mon''), TO_CHAR(DATE_BUCKET, ''IW''), NVL(' || v_lvl1_col || ', ''None''), NVL(' || v_lvl2_col || ', ''Unknown''), NVL(' || v_lvl3_col || ', ''Unknown'')';
    ...
    FOR date_rec IN cur_dates LOOP
        v_key := NULL;
        v_html := v_html || '<tr><td>' || date_rec.week_label || '</td>';
        FOR i IN 1 .. v_idx LOOP
...
create or replace FUNCTION OCI_FOCUS_REPORTS.cost_pivot_dynamic_monthly_html (
    ...
    CURSOR cur_dates IS
        SELECT DISTINCT TO_CHAR(TRUNC(DATE_BUCKET, 'MM'), 'Mon YYYY') AS month_label, TRUNC(DATE_BUCKET, 'MM') AS month_bucket
...
BEGIN
    ...
    v_sql := '
        ...
        || add_filter('RESOURCETYPE', p_resource_type)
        || add_filter('RESOURCEID', p_resource_name)
        || ' GROUP BY TRUNC(DATE_BUCKET, ''MM''), NVL(' || v_lvl1_col || ', ''None''), NVL(' || v_lvl2_col || ', ''Unknown''), NVL(' || v_lvl3_col || ', ''Unknown'')';
    ...
    -- Data rows
    FOR date_rec IN cur_dates LOOP
        v_html := v_html || '<tr><td>' || date_rec.month_label || '</td>';
        FOR i IN 1 .. v_idx LOOP
    ...
    FOR date_rec IN cur_dates LOOP
        ...
        FOR i IN 1 .. v_idx LOOP
            ...
            v_usage := CASE 
                    ...
                    END;
            v_html := v_html || '<td>' || CASE 
                                            WHEN v_usage < 1 THEN '0' || TO_CHAR(v_usage, 'FM.00') 
                                            ELSE TO_CHAR(v_usage, 'FM999G999G999G990D00') 
                                          END || '</td>';
            IF v_totals.EXISTS(v_columns(i)) THEN
...
--- New functionality - Create FOCUS_REPORTS_PY archive, move data and schedule monthly
CREATE TABLE OCI_FOCUS_REPORTS.FOCUS_REPORTS_PY_ARCHIVE
TABLESPACE DATA_ARCHIVE  -- or DATA if you don’t have a separate TS
...
AS
SELECT *
FROM   OCI_FOCUS_REPORTS.FOCUS_REPORTS_PY
WHERE  1 = 0;

CREATE INDEX OCI_FOCUS_REPORTS.IDX_FRP_ARCH_OCI_REF
  ON OCI_FOCUS_REPORTS.FOCUS_REPORTS_PY_ARCHIVE (OCI_REFERENCENUMBER)
  LOCAL
  TABLESPACE DATA_ARCHIVE;

CREATE OR REPLACE PROCEDURE OCI_FOCUS_REPORTS.ARCHIVE_FOCUS_REPORTS_PY (
...
BEGIN
  ...
  INSERT /*+ APPEND */
  INTO OCI_FOCUS_REPORTS.FOCUS_REPORTS_PY_ARCHIVE a
  SELECT *
  FROM   OCI_FOCUS_REPORTS.FOCUS_REPORTS_PY m
...
BEGIN
  DBMS_SCHEDULER.CREATE_JOB (
    ...
    job_action      => 'OCI_FOCUS_REPORTS.ARCHIVE_FOCUS_REPORTS_PY',
    ...
    enabled         => TRUE,
    comments        => 'Archive FOCUS_REPORTS_PY data older than 13 months, keeping main table to max 13 months of data.'
  );
END;
/
